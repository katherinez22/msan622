...
3. I tried to implement the comparison word cloud, which could definitely make the plot more complex. But I don't have a variable in the data set with fewer categories. The `Division` has 9 categories and the `State` has 50 categories. A comparison word cloud with 9 or more categories doesn't look very nice. I think the comparison word cloud would be better for a situation where there are fewer number of categories for a certain variable. Although the `Sex` has 2 categories in the data set, the baby names in these two categories are actually not correlated to each other. Hence, it will not make much sense to compare the baby names within these two categories.

## Challenges ##

There are some challenges I had during the whole process of this project. 

First, the original dataset has only 5 columns including `State`, `Sex`, `Year`, `Name` and `Number`, but it has over 10 million rows. It is a challenge for me to provide various interesting visualizations with limited number of data features. So I decided to extract other data to expand the number of variables in the data. Also, the total size of this data is over 100 MB, the maximum storing size of a single file in GitHub. So I ended up extracting only the latest 10 years data from the data set to carry out the visualization.

Second, I planned to show the stacked area plot for the baby names starting with specific characters that a user puts in the search box. Due to the incomplete record of information of some names in particular years and limitation of the data size, I was unable to provide the plot. But it was still very nice to try it. 
